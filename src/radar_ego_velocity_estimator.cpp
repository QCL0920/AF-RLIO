                                         ...
                                         sensor_msgs::PointCloud2& outlier_radar_msg)
{
  ...
  auto radar_scan_inlier(new pcl::PointCloud<RadarPointCloudType>);   auto radar_scan_outlier(new pcl::PointCloud<RadarPointCloudType>);
  ...
  if (1)   {
    ...
    for (uint i = 0; i < radar_scan->size(); ++i)
    {
      ...
      {
        Vector11 v_pt;         v_pt << target.x, target.y, target.z, target.intensity, -target.doppler * config_.doppler_velocity_correction_factor,
    ...
    if (valid_targets.size() > 2)
    {
      ...
      {
                Matrix radar_data(valid_targets.size(), 4);          uint idx = 0, idx_o = 0;
